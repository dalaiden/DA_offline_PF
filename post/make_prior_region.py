...
for var in var_list:
	...
	for i_season in range(len(list_seasons)):
		...
		# ----------loop to extract each EU region ---------------
		for i_region in range(len(list_regions)):

			region_id = list_regions[i_region]

			print('               - '+region_id)			

			# Load data
			fname = '{}/{}/{}/{}_{}-LE_{}_{}-{}_{}.nc'.format(loc_data_prior, model_ID, var_ID, var_ID, model_ID, season_id, years_prior[0], years_prior[-1], region_id)
			...
			for imember in range(data_all.shape[0]):
				...
				outfname = '{}/{}_prior_r{}i1p1_{}-{}_{}_{}.nc'.format(output_dir, var_ID, str(imember+1).zfill(3), year_a_prior, year_b_prior, season_id,region_id)
